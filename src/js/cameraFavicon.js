...
function drawNormal(ctx,S){
    ...
    ctx.fillStyle='#eee'; ctx.font = `${S*1}px serif`;
    ctx.textBaseline='middle'; ctx.fillText('∴', S*0.2, S*0.5);
}

function drawREC(ctx,S,{on=true}={}){
    ctx.fillStyle='#111'; ctx.fillRect(0,0,S,S);
    ctx.fillStyle='#eee'; ctx.font = `${S*0.32}px sans-serif`;
    ctx.textBaseline='middle'; ctx.fillText('REC', S*0.1, S*0.55);
    ctx.beginPath();
    ctx.fillStyle = on ? '#e33' : 'rgba(227,51,51,.25)';  // blink
    ctx.arc(S*0.82, S*0.22, S*0.12, 0, Math.PI*2); ctx.fill();
}
...
export function wireRecordingFavicon({blinkMs=700} = {}) {
    ...
    function startBlink() {
        if (timer) return;
        on = true; render();
        timer = setInterval(() => { on = !on; render(); }, blinkMs);
    }

    function stopBlink() {
        if (timer) { clearInterval(timer); timer = null; }
        on = false; forced = false; render();
    }
    ...
    return {
        enable() { forced = true; startBlink(); },
        disable() { forced = false; if (!document.hidden) stopBlink(); },
        pulse(ms=2000) { this.enable(); setTimeout(()=> this.disable(), ms); }
    };
}